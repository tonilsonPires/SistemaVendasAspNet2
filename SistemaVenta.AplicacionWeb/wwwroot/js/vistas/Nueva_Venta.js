...
$("#cboBuscarProducto").on("select2:select", function (e) {
  ...
  if (productoEncontrado.length > 0) {
    ...
    toastr.warning("", "Produto já existe")
    return false
  }
  ...
  swal({
    ...
    inputPlaceholder: "Insere a quantidade"
  },
    function (valor) {
      ...
      if (valor === "") {
        toastr.warning("", "Precisa inserir a quantidade")
        return false
      }

      if (isNaN(parseInt(valor))) {
        toastr.warning("", "Deves inserir um valor númerico")
        return false
      }
...
$("#btnTerminarVenta").click(function () {
  ...
  if (productosParaVenta.length < 1) {
    toastr.warning("", "Deves adicionar produtos")
    return
  }
  ...
  fetch("/Venta/RegistrarVenta", {
    ...
    .then(responseJson => {
      if (responseJson.estado) {
        ...
        swal("Registrado!", `Número da venda: ${responseJson.objeto.numeroVenta}`, "success")
      } else {
        swal("Lamentamos", "Não é possivel registrar venda ", "error")
      }
    })
